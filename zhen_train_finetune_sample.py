...
import cnfg.zhen_sample_finetune_base as cnfg
...
def train(low_td,middle_td,high_td,tl, ed, nd, optm, lrsch, model, lossf, mv_device, logger, done_tokens, multi_gpu, multi_gpu_optimizer, tokens_optm=32768, nreport=None, save_every=None, chkpf=None, state_holder=None, statesf=None, num_checkpoint=1, cur_checkid=0, report_eva=True, remain_steps=None, save_loss=False, save_checkp_epoch=False, scaler=None):
	...
	for i_d in tqdm(tl, mininterval=tqdm_mininterval):
		if rand_list[i_t] >= 0 and rand_list[i_t] < total_batch_num * 0.50:
			if int(i_d) < int(low_ntrain):
				...
				seq_batch = torch.from_numpy(low_src_grp[i_d][()])
				seq_o = torch.from_numpy(low_tgt_grp[i_d][()])
		if rand_list[i_t] >= total_batch_num * 0.50 and rand_list[i_t] < total_batch_num * 0.75:
			if int(i_d) < int(middle_ntrain):
				...
				seq_batch = torch.from_numpy(middle_src_grp[i_d][()])
				seq_o = torch.from_numpy(middle_tgt_grp[i_d][()])
		if rand_list[i_t] >= total_batch_num * 0.75 and rand_list[i_t] < total_batch_num:
