...
from loss.distill_double_base import LabelSmoothingLoss,nll_loss,distillloss
...
from transformer.distill_NMT_T10 import NMT
...
import cnfg.ende920404_double_distill_base4090_T10 as cnfg
from cnfg.ihyp import *
from cnfg.vocab.base import pad_id


def train(low_td,middle_td,high_td, tl, ed, nd, optm, lrsch, high_teach_model, teach_model,stu_model, lossf,distilllossf, mv_device, logger, done_tokens, multi_gpu, multi_gpu_optimizer,
    ...
    high_teach_model.eval()
    teach_model.eval()
    stu_model.train()
    cur_b, _ls = 1, {} if save_loss else None
    low_src_grp, low_tgt_grp = low_td["src"], low_td["tgt"]
    middle_src_grp, middle_tgt_grp = middle_td["src"], middle_td["tgt"]
    high_src_grp, high_tgt_grp = high_td["src"], high_td["tgt"]
    total_batch_num = len(tl)

    low_ntrain = low_td["ndata"][()].item()
    middle_ntrain = middle_td["ndata"][()].item()
    high_ntrain = high_td["ndata"][()].item()

    rand_list = [num for num in range(total_batch_num)]
    shuffle(rand_list)
    i_t = 0
    #tl 本身也是一个[0,1,2,3....1/3total]的batch_id集合，shuffle后也是集合id，下面的for循环在一个一个取batch_id
    for i_d in tqdm(tl, mininterval=tqdm_mininterval):
        if rand_list[i_t] >= 0 and rand_list[i_t] < total_batch_num * 0.92:
            if int(i_d) < int(low_ntrain):
                seq_batch = torch.from_numpy(low_src_grp[i_d][()])
                seq_o = torch.from_numpy(low_tgt_grp[i_d][()])
            else:
                i_d = str(int(i_d)-int(low_ntrain))
                seq_batch = torch.from_numpy(low_src_grp[i_d][()])
                seq_o = torch.from_numpy(low_tgt_grp[i_d][()])
        if rand_list[i_t] >= total_batch_num * 0.92 and rand_list[i_t] < total_batch_num * 0.96:
            if int(i_d) < int(middle_ntrain):
                seq_batch = torch.from_numpy(middle_src_grp[i_d][()])
                seq_o = torch.from_numpy(middle_tgt_grp[i_d][()])
            else:
                i_d = str(int(i_d) - int(middle_ntrain))
                seq_batch = torch.from_numpy(middle_src_grp[i_d][()])
                seq_o = torch.from_numpy(middle_tgt_grp[i_d][()])
        if rand_list[i_t] >= total_batch_num * 0.96 and rand_list[i_t] < total_batch_num:
            if int(i_d) < int(high_ntrain):
                seq_batch = torch.from_numpy(high_src_grp[i_d][()])
                seq_o = torch.from_numpy(high_tgt_grp[i_d][()])
            else:
                i_d = str(int(i_d) - int(high_ntrain))
                seq_batch = torch.from_numpy(high_src_grp[i_d][()])
                seq_o = torch.from_numpy(high_tgt_grp[i_d][()])
        lo = seq_o.size(1) - 1
        i_t += 1
        ...
        with torch_autocast(enabled=_use_amp):
            ...
            distill_loss = distilllossf(lossf,high_teach_out_softmax_withoutT,teach_out_softmax_withoutT,stu_out_softmax_withoutT,
                                        high_teach_out_sotfmax_byT,teach_out_sotfmax_byT, stu_out_sotfmax_byT, high_teach_out,teach_out, stu_out, ot)
            if multi_gpu:
...
def eva(ed, nd, high_teach_model,teach_model,stu_model, lossf, distilllossf, mv_device, multi_gpu, use_amp=False):
    r = w = 0
    sum_loss = 0.0
    teach_model.eval()
    high_teach_model.eval()
    ...
    with torch_inference_mode():
        for i in tqdm(range(nd), mininterval=tqdm_mininterval):
            ...
            with torch_autocast(enabled=use_amp):
                ...
                high_teach_out_softmax_withoutT, high_teach_out_sotfmax_byT, high_teach_out = high_teach_model(seq_batch, seq_o.narrow(1, 0, lo))
...
middle_td = h5File(cnfg.middle_train_data, "r")
high_td = h5File(cnfg.high_train_data, "r")
...
nword = low_td["nword"][()].tolist()

nwordi, nwordt = nword[0], nword[-1]

low_tl = [str(i) for i in range(low_ntrain)]
middle_tl = [str(i) for i in range(middle_ntrain)]
high_tl = [str(i) for i in range(high_ntrain)]



max_batch_id = max(len(low_tl),len(middle_tl),len(high_tl))
tl = [str(i) for i in range(max_batch_id)]
...
mymodel_teach = NMT(cnfg.isize, nwordi, nwordt, cnfg.nlayer, cnfg.ff_hsize, cnfg.drop, cnfg.attn_drop, cnfg.act_drop,
              cnfg.share_emb, cnfg.nhead, cache_len_default, cnfg.attn_hsize, cnfg.norm_output, cnfg.bindDecoderEmb,
              cnfg.forbidden_indexes)
mymodel_high_teach = NMT(cnfg.isize, nwordi, nwordt, cnfg.nlayer, cnfg.ff_hsize, cnfg.drop, cnfg.attn_drop, cnfg.act_drop,
              cnfg.share_emb, cnfg.nhead, cache_len_default, cnfg.attn_hsize, cnfg.norm_output, cnfg.bindDecoderEmb,
              cnfg.forbidden_indexes)
mymodel_teach.dec.T = mymodel_stu.dec.T = mymodel_high_teach.dec.T
fine_tune_m = cnfg.fine_tune_m

fine_tune_teach = cnfg.fine_tune_teach
fine_tune_high_teach=cnfg.fine_tune_high_teach
...
mymodel_high_teach = init_model_params(mymodel_high_teach)
mymodel_high_teach.apply(init_fixing)
...
if cuda_device:
    ...
    high_teach_model.to(cuda_device, non_blocking=True)
...
if multi_gpu:
    ...
    teach_model = DataParallelMT(teach_model, device_ids=cuda_devices, output_device=cuda_device.index, host_replicate=True,
                             gather_output=False)
    high_teach_model = DataParallelMT(high_teach_model, device_ids=cuda_devices, output_device=cuda_device.index, host_replicate=True,
                             gather_output=False)
...
if multi_gpu:
    ...
    high_teach_optimizer = high_teach_model.build_optimizer(Optimizer, lr=init_lr, betas=adam_betas_default, eps=ieps_adam_default,
                                        weight_decay=cnfg.weight_decay, amsgrad=use_ams,
                                        multi_gpu_optimizer=multi_gpu_optimizer,
                                        contiguous_parameters=contiguous_parameters)
    ...
    '''optimizer = Optimizer(get_model_parameters(mymodel, contiguous_parameters=contiguous_parameters), lr=init_lr,
                          ...
                          amsgrad=use_ams)'''
    high_teach_optimizer = Optimizer(get_model_parameters(high_teach_model, contiguous_parameters=contiguous_parameters), lr=init_lr,
                          betas=adam_betas_default, eps=ieps_adam_default, weight_decay=cnfg.weight_decay,
                          amsgrad=use_ams)
...
high_teach_model = torch_compile(high_teach_model, *torch_compile_args, **torch_compile_kwargs)
...
if fine_tune_teach is None:
    ...
    if cnt_states is not None:
        ...
        if "training_list" in _remain_states:
            ...
            shuffle(tl)
            _ctl = tl
        tminerr, done_tokens, cur_checkid, remain_steps, _ = train(low_td, middle_td, high_td, _ctl, vd, nvalid, stu_optimizer, stu_lrsch,high_teach_model, teach_model,student_model,
        ...
        vloss, vprec = eva(vd, nvalid, high_teach_model, teach_model,student_model, lossf, distilllossf, cuda_device, multi_gpu, use_amp)
