...
class distillloss(_Loss):
    ...
    def forward(self, cross_loss, high_teach_out_softmax_withoutT,teach_out_softmax_withoutT, stu_out_softmax_withoutT,
                high_teach_out_sotfmax_byT,teach_out_sotfmax_byT, stu_out_sotfmax_byT, high_teach_out, teach_out, stu_out, target, use_fast=True,use_new=False,use_avg=True,mask=None, **kwargs):
        # 两个软标签
        ...
        if _teach_predict_useful == 0:
            ...
            if use_new:
                ...
                if use_avg:
                    _loss = student_loss * self.B + ((distill_loss / _teach_predict_useful) * total_batch_word_num) * self.A + high_distill_loss * self.A
                else:
                    _loss = student_loss * self.B + distill_loss * self.A + high_distill_loss * self.A
            else:
                ...
                if use_avg:
                    ...
                    _loss = student_loss * self.B + distill_loss * self.A + high_distill_loss * self.C
        return _loss
