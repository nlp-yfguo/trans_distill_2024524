...
class DecoderLayer(DecoderLayerBase):
	...
	def __init__(self, isize, fhsize=None, dropout=0.0, attn_drop=0.0, act_drop=None, num_head=8, ahsize=None, norm_residual=norm_residual_default, k_rel_pos=use_k_relative_position_decoder, **kwargs):
		...
		self.ff_self = LSTMCell4FFN(isize, osize=isize, hsize=_fhsize, dropout=dropout, act_drop=act_drop)
		self.ff_cross = LSTMCell4FFN(isize, osize=isize, hsize=_fhsize, dropout=dropout, act_drop=act_drop)
	...
	def forward(self, inpute, inputo, cellin, src_pad_mask=None, tgt_pad_mask=None, query_unit=None, **kwargs):
		...
		context, cellout = self.ff_self(context, (lstmo, cellin))

		out = self.cross_attn(self.layer_normer2(context), inpute, mask=src_pad_mask)

		if self.drop is not None:
			out = self.drop(out)

		out, cellout = self.ff_cross(out, (context, cellout))
