...
from loss.distill_double_base_KLDloss import LabelSmoothingLoss,nll_loss,distillloss
...
import cnfg.zhen502525_A02B1C08_double_distill_base3090_KLDloss as cnfg
...
def train(low_td,middle_td,high_td, tl, ed, nd, optm, lrsch, high_teach_model, teach_model,stu_model, lossf,distilllossf, mv_device, logger, done_tokens, multi_gpu, multi_gpu_optimizer,
    ...
    for i_d in tqdm(tl, mininterval=tqdm_mininterval):
        if rand_list[i_t] >= 0 and rand_list[i_t] < total_batch_num * 0.25:
            if int(i_d) < int(low_ntrain):
                ...
                seq_batch = torch.from_numpy(low_src_grp[i_d][()])
                seq_o = torch.from_numpy(low_tgt_grp[i_d][()])
        if rand_list[i_t] >= total_batch_num * 0.25 and rand_list[i_t] < total_batch_num * 0.50:
            if int(i_d) < int(middle_ntrain):
                ...
                seq_batch = torch.from_numpy(middle_src_grp[i_d][()])
                seq_o = torch.from_numpy(middle_tgt_grp[i_d][()])
        if rand_list[i_t] >= total_batch_num * 0.50 and rand_list[i_t] < total_batch_num:
